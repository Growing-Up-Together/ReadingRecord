...
- 네스티드 루프 조인은 조인의 연결 조건이 되는 컬럼에 모두 인덱스가 있는 경우에 사용되는 조인 방식이다.
...
- 쿼리 실행 계획에서 Extra 컬럼에 “Using Join Buffer”라는 문구가 표시되면 조인 버퍼를 사용한다는 것을 의미한다.
...
- MySQL 5.6 버전부터는 인덱스를 범위 제한 조건으로 사용하지 못하더라도, 인덱스에 포함된 컬럼의 조건이 있다면 한꺼번에 모아서 스토리지 엔진으로 전달할 수 있다.
...
- InnoDB 스토리지 엔진은 프라이머리 키를 클러스터링 키로 생성한다. **그래서 모든 세컨더리 인덱스는 리프 노드에 프라이머리 키 값을 가진다.**
    ...
    - key_len 컬럼은 인덱스를 구성하는 컬럼 중에 어느 컬럼까지 사용했는지를 바이트 수로 보여준다.
...
- 쿼리 실행 계획에서 Extra 컬럼에 `Using intersect` 으로 표시된다.
    ...
    - `ix_firstname` 인덱스는 프라이머리 키인 emp_no 컬럼을 포함하고 있기 때문에 `ix_firstname` 인덱스만 사용하는 것이 더 성능이 좋을 것으로 생각할 수도 있다.
      → 서버 전체, 현재 세션, 현재 쿼리별로 인덱스 머지 최적화를 비활성화 할 수 있다.

**3.1.7 인덱스 머지 - 합집합 (index_merge_union)**

- 쿼리 실행 계획에서 Extra 컬럼에 `Using union` 으로 표시된다.
...
- Union 알고리즘에서 값이 중복되어 조회되거나, 중복되지 않는 경우에는 정렬 후 중복을 제거 했을 것이라고 예상한다.
  **→ 실제로는 프라이머리 키 컬럼의 값이 중복된 레코드를 정렬 없이 걸러낼 수 있게 해준다. 프라이머리 키로 정렬된 두 집합의 결과를 가져와 우선순위 큐 알고리즘을 사용하여 중복을 제거한다.**
...
- 인덱스 머지 작업을 하는 도중에 결과의 정렬이 필요한 경우 MySQL 서버는 인덱스 머지 최적화의 ‘Sort union’ 알고리즘을 사용한다.
    ...
    - 첫번째 쿼리 결과는 `emp_no`로 정렬되어 출력되지만, 두 번째 쿼리 결과는 `emp_no` 컬럼으로 정렬되어 있지 않기 때문에 우선순위 큐를 사용하여 중복을 제거하는 것이 불가능하다.
    - 이런 경우에는 두 집합의 결과에서 중복을 제거하기 위해 각 집합을 `emp_no` 컬럼으로 정렬한 다음 중복 제거를 수행한다.
    - 중복을 제거하기 위해 강제로 정렬을 수행해야 하는 경우에는 실행 계획의 Extra 컬럼에 `Using sort_union` 으로 표시된다.
...
- 테이블 풀-아웃 최적화는 세미 조인의 서브쿼리에 사용된 테이블을 아우터 쿼리로 끄집어낸 후에 쿼리를 조인 쿼리로 재작성하는 형태의 최적화이다.
    ...
    - id 컬럼의 값이 모두 `1`인 것은 두 테이블이 서브쿼리 형태가 아니라 조인으로 처리되었음을 의미한다.
- 테이블 풀-아웃 최적화는 실행 계획의 Extra 컬럼에 별도로 표시되지 않기 때문에, 테이블 풀-아웃 최적화가 사용되었는지 확인하려면 id 컬럼 값이 같은지 확인하거나, `SHOW WARNINGS` 명령으로 옵티마이저가 재작성한 쿼리를 살펴봐야 한다.
...
- `IN(subquery)` 형태의 세미 조인을 `EXISIT(subquery)` 형태로 튜닝한 것과 비슷한 방법으로 실행된다.
    ...
    - id 컬럼의 값이 모두 `1`이고, Extra 컬럼에는 `FirstMatch(e)`로 표시된다.
...
- 인덱스를 사용하는 `GROUP BY` 최적화 방법에서 살펴본 “Using index for group-by”의 루스 인덱스 스캔과 비슷한 읽기 방식을 사용한다.
    ...
    - id 컬럼의 값이 모두 `1`이고, Extra 컬럼에는 `LooseScan`으로 표시된다.
...
- 세미 조인에 사용된 서브쿼리를 내부 임시 테이블을 생성하여 구체화해서 쿼리를 최적화한다는 의미이다.
    ...
    - 실행 계획의 selected_type 컬럼에 `MATERIALIZED`로 표시된다.
...
- 실행 계획의 Extra 컬럼에 별도로 표시되지 않지만, `Start temporary`와 `End temporary`가 표시된다.
...
- MySQL 옵티마이저는 여러 테이블이 조인되는 경우 가능하면 일치하는 레코드 건수가 적은 순서대로 조인을 실행한다.
    ...
    - 컨디션 팬아웃 최적화가 비활성화된 경우에는 실행 계획의 filtered 컬럼의 값이 100%인데, 이는 `employees` 테이블에서 `first_name = 'Matt'` 조건에 일치하는 233건의 레코드가 모두 `hire_date` 컬럼의 조건을 만족할 것이라고 예측했다.
    - 컨디션 팬아웃 최적화가 활성화된 경우에는 filtered 컬럼의 값이 23.20%인데, 이는 `employees` 테이블에서 54건(233 * 0.2320)의 레코드가 hire_date 컬럼의 조건을 충족할 것으로 예측한 것이다.
- 컨디션 팬아웃 최적화 비율 예측
...
- `FROM` 절에 사용된 서브쿼리를 파생 테이블이라고 부른다.
    ...
    - 이 쿼리의 경우 내부적으로 임시 테이블으 생성하고 `first_name='Matt'`인 레코드들만 읽어서 임시 테이블로 INSERT 하고 다시 임시 테이블을 읽으므로, 레코드를 복사하고 읽는 오버헤드가 추가된다.
...
- 인덱스 스킵 스캔은 인덱스의 선행 컬럼이 조건절에 사용되지 않더라도 후행 컬럼의 조건만으로도 인덱스를 이용한 쿼리 성능 개선이 가능하도록 한다.

    ```sql
    ALTER TABLE employees ADD INDEX ix_gender_birthdate (gender, birth_date);
    
    -- // gender 컬럼이 없더라도 인덱스 스킵 스캔으로 인덱스를 사용하여 조회
    ...
    - 옵티마이저는 인덱스의 선행 컬럼이 소수의 유니크한 값을 가질 때만 인덱스 스킵 스캔 최적화를 사용한다.
...
- 실행 계획의 Extra 컬럼에 `hash join`으로 표시된다.
...
- STRAIGHT_JOIN 힌트 사용 예시
    ...
    - **일반 테이블끼리 조인하는 경우**
    - 두 테이블의 조인 컬럼에 모두 인덱스가 있거나 없는 경우에는 스캔되는 레코드 수에 따라 옵티마이저가 적절히 드라이빙 테이블을 선택한다.
    - 한 테이블의 조인 컬럼에만 인덱스가 있는 경우에는 **반복된 풀 스캔을 막기 위해 인덱스가 없는 테이블을 드라이빙으로 선택하는 것이 좋다.**
...
- 인덱스 힌트 주의사항
    - 옵티마이저는 프라이머리 키나 전문 검색 인덱스에 대해서 가중치를 두고 실행계획을 수립한다.
      그래서 일반 보조 인덱스를 사용할 수 있는 상황이더라도 전문 검색 인데스를 선택할 수 있다.
...
- `SEMIJOIN` 힌트는 어떤 세부 전략을 사용할지 제어할 수 있다.
  ...
  ![9-17](../images/winnie/chapter9/9-17.png)
  ...
  ![9-18](../images/winnie/chapter9/9-18.png)
...
- 조인 조건이 되는 컬럼의 인덱스가 적절히 준비되어 있으면 해시 조인은 거의 사용되지 않는다.
...
- 일부 테이블의 조인 순서만 결정하고 나머지 테이블은 옵티마이저가 결정하도록 다른 힌트가 추가되었다.
    ...
    ```sql
    -- // FROM 절에 나열된 테이블의 순서대로 조인 (d → de → e)
    EXPLAIN
...
- 하지만 인덱스 컨디션 푸시다운으로 인해 여러 실행 계획의 비용 계산이 잘못된다면 결과적으로 잘못된 실행 계획을 수립하게 될 수 있으므로 ICP을 비활성화해서 유연하게 실행 계획을 선택할 수 있다.
    ...
    -- // Extra 컬럼의 "Using inddex condition"은 기본적으로 ICP 최적화를 선택한 것
    EXPLAIN
    SELECT *
    FROM employees
...
- [[SQL 튜닝] 드라이빙 테이블(DRIVING TABLE)의 개념/결정 규칙]([https://devuna.tistory.com/36](https://devuna.tistory.com/36))
