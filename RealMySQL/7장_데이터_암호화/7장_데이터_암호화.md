...
- 암호화된 테이블의 경우 원본 서버와 목적지 서버의 마스터 키 가 다르기 때문에 `FLUSH TABLES` 명령을 실행해야 한다. 
    - 일반적인 테이블스페이스 Export  
      ```text
    ...
    - 암호화된 테이블스페이스 Export  
      ...
      ② 임시로 사용할 마스터 키를 발급해서 source_table.cfg 파일에 기록
      ③ 암호화된 테이블의 테이블스페이스 키를 기존 마스터 키로 복호화  
      ④ 임시로 발급한 마스터 키를 이용해 다시 암호화해서 데이터 파일의 헤더 부분에 저장  
      ⑤ 복사가 완료되면 소스 테이블의 잠금 해제
      ```  
      ➡️ 암호화된 테이블의 경우 테이블스페이스 이동 기능을 사용할 때 데이터 파일과 `*.cfg` 파일을 함께 복사해야 한다. 
