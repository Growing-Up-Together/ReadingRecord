...
- 설치 후 MySQL 서버가 자동으로 실행된다
  ...
  ![mysql서버프로세스확인](./images/chawon_1.png)
...
- telnet 으로 접속 확인
![telnet 접속확인](./images/chawon_2.png)
...
- 서버가 어느 디렉터리에서 `my.cnf` 파일을 읽는지 궁금하다면 
  ...
  mysqld --verbose --help
  ```
  ![--verbose--help](./images/chawon_3.png)
...
- `SHOW` 명령어를 통해 변수를 보거나 `SET` 명령어를 통해 변수를 변경할 수 있다.
  ...
  ```
  ![max_connections](./images/chawon_4.png)
- `SET PERSIST` 명령어로 시스템 변수를 변경시 `mysqld-auto.cnf`, `performance_schema.variables_info`, `performance_schema.persisted_variables` 에 기록이 남는다.
  ![mysqld-auto.cnf](./images/chawon_5.png)
  ![mysqld-auto.cnf](./images/chawon_6.png)
...
- 위 명령어로 추가된 시스템 변수를 삭제하려면 `RESET PERSIST` 명령어를 활용한다.
  ```mysql
  RESET PERSIST [옵션명]; -- 옵션명을 안넣을시 mysqld-auto.cnf 모든 시스템변수를 삭제
  ```
