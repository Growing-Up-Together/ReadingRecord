...
- 사용자에게 권한을 부여할 때는 `GRANT` 명령어를 사용한다.
  ...
  GRANT EVENT ON subway.* TO 'user'@'localhost';
  
  -- 테이블 권한
  GRANT SELECT, INSERT, UPDATE, DELETE ON *.* TO 'user'@'localhost';
  GRANT SELECT, INSERT, UPDATE, DELETE ON subway.* TO 'user'@'localhost';
  GRANT SELECT, INSERT, UPDATE, DELETE ON subway.member  TO 'user'@'localhost';
  GRANT SELECT, INSERT, UPDATE(id), DELETE ON subway.member  TO 'user'@'localhost';
  ``` 
    - 글로벌 권한의 `ON` 절에는 항상 `*.*` 를 사용하게 된다.
    - `*.*`은 모든 DB의 모든 오브젝트(테이블과 스토어드 프로시저나 함수 등)를 포함해서 MySQL 서버 전체를 의미한다.
    - DB 권한 범위는 테이블에 대해 부여할 수 없다.   
      **e.g.** `GRANT EVENT ON subway.member TO 'user'@'localhost';`
